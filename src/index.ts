...
const DATA_PATH = 'test_account/baseblu'
const storage = new CloudStorage(DATA_PATH)
...
list
  ...
  .then(() => {
    ...
    const ScraperModule = require(path.join(
      __dirname,
      `./scrapers/${scraperName}`
    )).default
    const scraper: Scraper = new ScraperModule(process.argv, list)
    ...
    from(list.urls)
      .pipe(
        ...
        map((obj, i) => ({
          ...obj,
          price_ref: {
            formula: obj.price_ref_formula.replace(/_/g, '' + (i + 2))
          },
          price_pro: {
            formula: obj.price_pro_formula.replace(/_/g, '' + (i + 2))
          },
